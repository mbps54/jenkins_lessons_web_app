    ...
    stage('Deploy') {
      ...
      steps {
        ...
        sshagent(credentials: ['ubuntu-creds']) {
          sh "ssh -o StrictHostKeyChecking=no ubuntu@18.197.105.36 'cd web_app && sudo systemctl stop web_app && rm -f web_app && wget -O web_app https://mbps54-bucket-artif-1.s3.eu-central-1.amazonaws.com/pipeline_lesson_10/${env.BUILD_NUMBER}/artifacts/web_app && sudo chmod +x web_app && sudo systemctl start web_app'"
        }
      }
    }
  }
